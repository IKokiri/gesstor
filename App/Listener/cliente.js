...
interact_fields['uf'] = '';
interact_fields['nome_reduzido'] = '';
interact_fields['numero_sapiens'] = '';
        ...
        data: {
            ...
            if (data.count) {
                ...
                $.each(data.result, function (key, value) {
                    ...
                    if (value.nome_reduzido) {
                        cliente = value.nome_reduzido + " - " + value.cnpj;
                    } else {
                        cliente = value.identificador;
                    }
                    tbody += '<tr>' +
                        '<td width="15%">' + value.tipo_pessoa + '</td>' +
                        '<td width="30%">' + cliente + '</td>' +
                        '<td width="30%">' + value.cnpj + '</td>' +
                        '<td width="30%">' + value.bairro + ', ' + value.cidade + ' - ' + value.uf + '</td>' +
...
function getContatosCliente(formData) {
...
function loadServicosCliente(formData) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.result) {
                ...
                $.each(data.result, function (key, value) {
                    servicos += "<div data-idservicocliente='" + value.id + "' class='col-md-2 col-sm-2 col-xs-2 servicoDoCliente " + value.icone + " fa-4x' title='" + value.servico + " R$ " + value.valor_servico + " " + value.identificador + "'></div>";
                });
...
$(document).ready(function () {
    ...
    $(document).on("click", ".servicoDoCliente", function () {
